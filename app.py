...
app.config['MAIL_PASSWORD'] = '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'
app.config['MAIL_DEFAULT_SENDER'] = 'vijay.annamalai001@gmail.com'
mail = Mail(app)
cors = CORS(app)
...
conn=ibm_db.connect("DATABASE=bludb;HOSTNAME=ea286ace-86c7-4d5b-8580-3fbfa46b1c66.bs2io90l08kqb1od8lcg.databases.appdomain.cloud;PORT=31505;Security=SSL;SSLServerCertificate=DigiCertGlobalRootCA.crt;UID=cll91382;PWD=kjFUyCZ8AYMfl7jp;","","")
...
#         sql = "SELECT * FROM PETA_USER WHERE name =?"
#         stmt = ibm_db.prepare(conn, sql)
...
#         else:
#             insert_sql = "INSERT INTO  PETA_USER VALUES (?, ?, ?)"
...
def update_balance():
    if request.method == 'GET':
        ...
        global USERID
        if EMAIL == '':
            return render_template('signin.html', msg='Login before proceeding')
        if(USERID==''):
...
def add_expense():
    if request.method=='GET':
        ...
        return render_template('Expense.html', categories=categories, groups=groups)
